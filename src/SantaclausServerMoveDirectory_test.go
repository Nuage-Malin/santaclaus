	...
	for _, dir := range index.SubFiles.DirIndex {
		if dir.DirId == request.DirId {
			if dir.ApproxMetadata.DirPath != filepath.Join(createDirReq.Directory.DirPath, createDirReq.Directory.Name) {
				t.Fatalf("Directory path has not been moved properly, it is %s, but should be %s", dir.ApproxMetadata.DirPath, filepath.Join(createDirReq.Directory.DirPath, createDirReq.Directory.Name))
			}
		}
	}
}
